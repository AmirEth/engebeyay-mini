    ...
    if current_user.is_authenticated:
        ...
        if is_cart_item_exists:
            ...
            if product_variation in ex_var_list:
                ...
                # check if stock in the product is finished
                cart_product = Product.objects.get(product_id=product)
                quant = cart_product.stock
                if item.quantity < quant:
                    item.quantity += 1
                    item.save()
                else:
                    messages.error(
                        request, 'your quantity excceded the stock value, you cannot add more!.')
            else:
