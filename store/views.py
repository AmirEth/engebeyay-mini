...
def product_detail(request, category_slug, product_slug):
    ...
    if request.user.is_authenticated:
        try:
            orderproduct = OrderedProduct.objects.filter(
...
@buyer_required
def fov_item(request):
    user = request.user
    fov_item = Favorite.objects.filter(user=user)
    context = {"fov_items": fov_item}
    return render(request, 'store/favorite_items.html', context)
